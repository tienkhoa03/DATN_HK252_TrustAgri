...
/**
 * Khi TRACEABILITY_INTERNAL_SECRET được cấu hình, chỉ chấp nhận request có header khớp
 * (farm-service gọi nội bộ). Khi không cấu hình — môi trường dev.
 */
@Injectable()
export class TraceabilityInternalGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (value !== secret) {
      throw new ForbiddenException('Truy cập biểu đồ cảm biến traceability không hợp lệ');
    }
    return true;
  }
}
